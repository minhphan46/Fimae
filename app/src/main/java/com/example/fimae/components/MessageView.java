...
public class MessageView extends ConstraintLayout {
    ...
    public void setMessage(Message message) {
        ...
        if (Objects.equals(message.getType(), Message.TEXT)) {
            ...
            ShortsRepository.getInstance().getShortById(message.getContent().toString()).addOnSuccessListener(shorts -> {
                if (shorts.getMediaUrl() != null && !shorts.getMediaUrl().toString().isEmpty()) {
                    postMessage.setImage(shorts.getMediaUrl().toString());
                } else {
                    postMessage.setImageVisibility(View.GONE);
                }
                String content = shorts.getDescription();
                postMessage.setTitle(content);
            });
            postMessage.setLabelButton("XEM VIDEO");
            postMessage.setOnClickListener(v -> {
                Intent intent = new Intent(getContext(), ShortVideoActivity.class);
                intent.putExtra("idVideo", message.getContent().toString());
                getContext().startActivity(intent);
            });
            }
        }
